...
function extractVariants(content: string): Record<string, any> {
  ...
  // Component name patterns to try (in order of preference)
  const namePatterns = [
    // Try to extract the component name from different export patterns
    { pattern: /export const ([a-zA-Z]+)Variants\s*=\s*{/s, suffix: 'Variants' },
    { pattern: /export const ([a-zA-Z]+)Demos\s*=\s*{/s, suffix: 'Demos' },
    { pattern: /export const ([a-zA-Z]+)Examples\s*=\s*{/s, suffix: 'Examples' }
  ];
  ...
  for (const { pattern, suffix } of namePatterns) {
    const match = content.match(pattern);
    if (match) {
      componentName = match[1].toLowerCase();
      
      // Now find the full export content with opening and closing braces
      const fullExportMatch = content.match(new RegExp(`export const ${match[1]}${suffix}\\s*=\\s*{([\\s\\S]*?)}`, 's'));
      if (fullExportMatch) {
        exportContent = fullExportMatch[1];
        matchFound = true;
        break;
      }
    }
  ...
  if (matchFound && exportContent) {
    // Extract all key-value pairs from the variants object
    // Look for patterns like: demo: ComponentDemo, variants: ComponentVariants, etc.
    const variantPairs = exportContent.match(/([a-zA-Z0-9_]+)\s*:\s*([a-zA-Z0-9_]+)/g);
    ...
    if (variantPairs) {
      for (const pair of variantPairs) {
        ...
        if (key && value) {
          variants[key.trim()] = {
            name: key.trim(),
            description: `${key.trim()} variant of ${componentName}`,
...
async function buildRegistryItems(validatedRegistry: Registry) {
  ...
  for (const item of [...uiComponents, ...examples, ...blocks]) {
    ...
    const componentData = {
      ...
      css: item.css,
      // @DEPRECATED: Include tailwind config for backward compatibility with v3
